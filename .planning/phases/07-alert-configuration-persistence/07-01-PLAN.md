...
must_haves:
  truths:
    ...
    - "The getAlerts query reads thresholds from alertConfig (with hardcoded defaults as fallback) and filters out dismissed alerts for the requesting user"
  artifacts:
    ...
    - path: "convex/alerts.ts"
      provides: "getAlerts query using dynamic config and dismissal filtering"
      exports: ["getAlerts"]
  key_links:
    ...
    - from: "convex/alerts.ts"
      to: "convex/schema.ts (alertDismissals table)"
      via: "ctx.db.query('alertDismissals') filtered by userId"
      pattern: "alertDismissals"
...
Output: Four modified/created Convex files providing alertConfig CRUD, alertDismissals CRUD, and a refactored getAlerts query that uses dynamic config + dismissal filtering.
</objective>
